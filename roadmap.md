...
This document provides a comprehensive roadmap for extending the current user authentication and
management service into an enterprise-grade SaaS user module.
...
**Database**: Add columns to `users` table (isLocked, lockedAt, lockedReason, failedLoginAttempts,
lastFailedLoginAt)
...
**Database**: `user_sessions` table (id, userId, refreshTokenId, deviceName, browser, os, ipAddress,
location, lastActivityAt, createdAt)
...
**Database**: `social_accounts` table (id, userId, provider, providerId, email, profilePicture,
accessToken, refreshToken, createdAt, updatedAt)
...
**Security**: Handle email matching, require verification for account linking, make User.password
nullable
...
**Database**: `api_keys` table (id, userId, tenantId, name, keyHash, prefix, permissions, expiresAt,
...
**Security**: Store hashed keys, show full key only on creation, support key prefixes for
identification
...
**Database**: `audit_logs` table (id, tenantId, userId, action, resource, resourceId, status,
ipAddress, userAgent, details JSONB, createdAt)
...
**Database**: `user_preferences` table (userId, language, timezone, dateFormat, emailNotifications
JSONB, themeMode, updatedAt)
...
**Database**: Add to `users` table (isMfaEnabled, mfaSecret encrypted, mfaBackupCodes encrypted,
mfaEnabledAt)
...
**Authentication Flow**: Login returns `mfaRequired: true`, then verify with
`POST /api/users/auth/mfa-verify`
...
- Revisit spring-grpc when it's 1.0, then add grpc tests
