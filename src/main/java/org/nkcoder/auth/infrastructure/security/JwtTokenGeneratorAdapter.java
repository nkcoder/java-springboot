...
public class JwtTokenGeneratorAdapter implements TokenGenerator {
    ...
    @Override
    public TokenPair generateTokenPair(AuthUserId userId, Email email, AuthRole role, TokenFamily tokenFamily) {
    ...
    public RefreshTokenClaims validateRefreshToken(String token) {
        try {
            ...
            AuthUserId userId = AuthUserId.of(claims.getSubject());
    ...
    private String generateAccessToken(AuthUserId userId, Email email, AuthRole role) {
        Date now = new Date();
    ...
    private String generateRefreshToken(AuthUserId userId, TokenFamily tokenFamily) {
        Date now = new Date();
        Duration duration = parseDuration(jwtProperties.expiration().refresh());
        Date expiration = new Date(now.getTime() + duration.toMillis());
