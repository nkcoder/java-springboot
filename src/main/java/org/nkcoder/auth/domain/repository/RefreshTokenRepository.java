...
/** Repository interface (port) for RefreshToken persistence. Implementations are in the infrastructure layer. */
public interface RefreshTokenRepository {

    Optional<RefreshToken> findByToken(String token);

    /**
     * Finds a refresh token by its value with a pessimistic lock for update. Used during token refresh to prevent race
     * conditions.
     */
