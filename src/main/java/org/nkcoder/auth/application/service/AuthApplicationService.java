...
public class AuthApplicationService {
    ...
    public AuthResult register(RegisterCommand command) {
        ...
        // Create auth user
        AuthUser authUser =
                AuthUser.register(email, passwordEncoder.encode(command.password()), command.name(), command.role());
        ...
        eventPublisher.publish(new UserRegisteredEvent(authUser.getId(), email, command.name(), authUser.getRole()));

        // Generate tokens
        TokenFamily tokenFamily = TokenFamily.generate();
        TokenPair tokens = tokenGenerator.generateTokenPair(authUser.getId(), email, authUser.getRole(), tokenFamily);

        // Save refresh token
        saveRefreshToken(tokens.refreshToken(), authUser, tokenFamily);

        return AuthResult.of(authUser.getId().value(), authUser.getEmail().value(), authUser.getRole(), tokens);
    }
    ...
    public AuthResult refreshTokens(RefreshTokenCommand command) {
        ...
        try {
            ...
            TokenPair tokens = tokenGenerator.generateTokenPair(
    ...
    private void saveRefreshToken(String token, AuthUser authUser, TokenFamily tokenFamily) {
        RefreshToken refreshToken =
                RefreshToken.create(token, tokenFamily, authUser.getId(), tokenGenerator.getRefreshTokenExpiry());
