...
public record JwtProperties(
    ...
    public record Expiration(
            @NotBlank @Pattern(regexp = "\\d+[smhd]", message = "Access token expiration must match pattern: <number>[s|m|h|d]") String access,

            @NotBlank @Pattern(regexp = "\\d+[smhd]", message = "Refresh token expiration must match pattern: <number>[s|m|h|d]") String refresh) {
